...
public class Door : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Channel to use to connect to buttons. Buttons need to have the same channel to open the door")]
    private int channel = 1;
    [SerializeField] private int buttonsNedded = 1;

    [Header("Position Settings")]
    [SerializeField]
    [Tooltip("Position of the door when it opens")]
    ...
    void Update()
    {
        if (shouldOpen)
        {
            transform.position = Vector3.MoveTowards(transform.position, openPosition.position, Time.deltaTime);
        }
